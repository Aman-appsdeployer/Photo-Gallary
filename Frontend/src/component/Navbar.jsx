...
function ThemeToggle() {
  ...
  return (
    <button
      ...
      className="w-9 h-9 flex items-center justify-center border border-rose-500 text-rose-500 rounded-full hover:bg-rose-500 hover:text-black transition"
    >
...
function Navbar() {
  ...
  return (
    <header className="fixed top-0 w-full z-50 bg-black/80 backdrop-blur-xl border-b border-cyan-400/30">
      <div className="container mx-auto px-4 py-3 flex items-center justify-between">
        ...
        {/* DESKTOP NAV */}
        <nav className="hidden lg:flex space-x-8 text-white text-lg">
          {menuItems.map((item) => (
            <button
              key={item.path}
              onClick={() => go(item.path)}
              className={`transition ${
                activeTab === item.path ? "text-rose-500" : "hover:text-rose-500"
              }`}
            >
              {item.label}
            </button>
          ...
          {token && (
            <button
              onClick={() => go("/profile")}
              className={`transition ${
                activeTab === "/profile" ? "text-rose-500" : "hover:text-rose-500"
              }`}
            >
              Profile
            </button>
          ...
          <div className="hidden lg:flex items-center gap-3">
            {!token ? (
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 rounded-lg bg-rose-500 text-black font-semibold hover:opacity-90 transition"
              >
                <LogIn size={18} />
                Login
              </button>
              ...
              <button
                onClick={logout}
                className="flex items-center gap-2 px-4 py-2 rounded-lg bg-red-500 text-white font-semibold hover:opacity-90 transition"
              >
                <LogOut size={18} />
                Logout
              </button>
              ...
              <>
                ...
                >
                  <Popover.Panel className="absolute top-0 left-0 w-full z-50">
                    ...
                    <div className="relative bg-gray-900 w-full p-6 shadow-2xl border-b border-cyan-400/40">
                      ...
                      <ul className="space-y-5 text-white text-2xl font-bold">
                        {menuItems.map((item) => (
                          <li key={item.path}>
                            <button
                              onClick={() => go(item.path, close)}
                      ...
                      <div className="mt-7 flex gap-4">
                        {!token ? (
                          <button
                            onClick={() => go("/login", close)}
                            className="px-4 py-2 bg-rose-500 text-black rounded-lg font-semibold"
                          >
                            Login
                          </button>
                        ) : (
